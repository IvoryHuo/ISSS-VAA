...
-   The **label of the x-axis** is hard to read. In fact, the gender can be represented by legend and the 'population' can be labeled just once.

-   The **1st dashboard** include overview of the Singapore population by PA and the age-sex pyramid of Singapore.Those diagrams should be displayed together with the main trellis at margin as reference.

-   The **3rd dashboard** include pyramids of 9 sub zones. It should be removed as the requirement states that the pyramids should be drawn at planning area level.
...
```
4. Note that the shape of the pyramid above is exactly as the original one. We notices that the order of the age group on y-axis is wrong. Hence, we need to reorder the age groups on y-axis as shown below.
Moreover, the 9 most populated PA is arranged according to the mean age in descending order.
```{r}
