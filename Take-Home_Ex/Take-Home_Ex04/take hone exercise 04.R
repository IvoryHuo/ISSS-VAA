...
#| warning: false
#| message: false
pacman::p_load(ggbraid,imager,readxl,lubridate,zoo,plotly,GGally,DT, patchwork,ggHoriPlot,CGPfunctions,gganimate,tidyverse)
...
3.4 Analyzing 3 measures using Ternary Plot
...
{r}
# Note that sometimes we got an error msg when we load ggtern together with tidyverse
library(ggtern)
tern_data <- slope_yr %>%
  mutate(sur_def = ifelse(yr_bal >0, "pos", "neg")) %>%
  mutate(imp_exp_tot = yr_import + yr_export)
tern_data

{r}
plot_tern <- function(the_ct,the_yr) {
  tern <- tern_data %>%
    filter(Year == the_yr) %>%
    filter(country == the_ct) %>%
    ggtern(aes(x = yr_import,y=yr_export,z=yr_bal)) +
    geom_point() +
    guides(size = none) +
    theme_bw() +
    ggtitle(paste0("Composition of Trade between ", the_ct, " and Singapore in ", the_yr))
  tern
}


{r}
plot_tern("Mainland China",2022)


3.5 Animate Trade Performance between Singapore and its partners using Bubble Chart
...
bubble_ani<- animate(bubble_plot)
anim_save('trade_animation.gif',bubble_ani)
